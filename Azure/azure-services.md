...
| **Azure Container Instances (ACI)** | Official Microsoft description: *"Launch containers with hypervisor isolation."* A raw, single/simple-container compute primitive with zero orchestration — no cluster to create or manage; each container group gets hypervisor-level isolation (a real security boundary, not just a shared-kernel namespace). Per-second billing for vCPU/memory actually consumed; no built-in scaling across instances, self-healing, revisions, or HTTP ingress. | Burst compute, simple batch jobs, CI/CD build agents, or as a building block invoked *by* other services rather than a standalone app platform. | You need a one-off or simple container workload without orchestration overhead — much lower operational complexity than AKS, but no built-in scaling/self-healing across many containers. |
...
| **Azure Container Apps** | Official Microsoft description: *"Run modern apps and agents using serverless containers."* A fully managed serverless *application platform* built on Kubernetes, KEDA, Dapr, and Envoy under the hood — but the cluster/API is never exposed to you. Adds everything ACI lacks: HTTPS ingress with automatic TLS, revisions with traffic-splitting (blue/green, canary), scale-to-zero, KEDA-based event-driven autoscaling, and Dapr for service-to-service calls/pub-sub/state. Consumption-based billing (vCPU-seconds, memory, requests) with a free monthly grant. | Microservices, event-driven apps needing container flexibility without managing Kubernetes. | You want Kubernetes-like scaling and traffic-splitting features without operating a cluster — simpler and often cheaper than AKS for teams that don't need raw k8s API access. |
