...
| JD Requirement | Definition & Explanation | Use Case & Application | Your Standing | Prep Needed |
|---|---|---|---|---|
| **AWS + Terraform certified, CKA** (listed as "preferred") | Third-party-validated credentials proving hands-on competency at a defined level — AWS certs are tiered (Foundational → Associate → Professional/Specialty), Terraform Associate validates core HCL/state/workflow knowledge, and CKA (Certified Kubernetes Administrator) is a hands-on, in-cluster practical exam (not multiple-choice), widely regarded as harder to earn than most cloud certs precisely because of that format. | Signals baseline competency to a hiring team without requiring them to fully assess raw skill from a resume alone — directly satisfies (and exceeds) this JD's "preferred" bullet, a credential-based talking point to lead the interview with. | **Exceeds it** — AWS Solutions Architect Professional, AWS Security Specialty, Terraform Associate, CKA, plus SA Associate/SysOps/Developer Associate/Cloud Practitioner | None — lead with this. Most candidates meeting the "preferred" bar have one cert; you clear it by a wide margin. |
| Kubernetes, 2+ years | An open-source container orchestration platform that automates deploying, scaling, healing, and networking containerized workloads across a cluster of machines — the industry-standard way to run containers in production at scale, with Amazon EKS being AWS's managed control-plane offering of it. | Hosting Data Science platform services on EKS instead of hand-managed EC2 fleets — e.g., custom inference endpoints, internal platform tooling, or batch/Spark-style processing jobs, all benefiting from self-healing and declarative scaling rather than manual instance management. | **Strong** — EKS multi-tenant RBAC/namespace isolation on resume, CKA-certified | None. |
| Terraform / IaC | HashiCorp's open-source, provider-agnostic Infrastructure-as-Code tool — you declare desired infrastructure state in HCL, and Terraform computes and executes the create/update/destroy operations to reach it, tracking what it manages in a state file. "IaC" (Infrastructure as Code) is the broader practice this and tools like CloudFormation both implement: provisioning infrastructure via versioned, reviewable code instead of manual console clicks. | Provisioning per-team/per-environment AWS accounts and resources for a growing Data Science org — precisely the JD's "develop and deploy AWS environments using CI/CD" bullet, reused as a template each time a new team/environment is requested rather than hand-built. | **Strong** — modules, remote state, policy gates all on resume | None. |
| Azure DevOps CI/CD | Microsoft's application lifecycle platform bundling Git repos, YAML-based CI/CD pipelines, work-item tracking, and artifact feeds — commonly used to orchestrate build/test/deploy pipelines even for non-Azure targets (like the AWS environments this JD describes), via service connections that authenticate out to the target cloud. | The specific pipeline tool this JD names for deploying AWS environments — running `terraform plan`/approval-gate/`terraform apply` stages, and gating promotion between dev/uat/prod AWS accounts via Environments. | **Strong** — explicitly named on resume (Tech Consulting role) | None. |
| Python / shell scripting | Python: a general-purpose, readable scripting language, the dominant choice for AWS/cloud automation (via Boto3), data engineering (PySpark/pandas), and Lambda functions. Shell scripting (Bash): the default Linux automation glue for deployment steps, log collection, and general system tasks. | Writing the "scripts and applications" this JD explicitly calls out — automating AWS account setup, ETL glue code around Glue/Athena, and operational/on-call remediation scripts. | **Strong** — Python (PySpark, pandas), Bash | None. |
| Docker | A container runtime and image-format standard — packages an application with its dependencies into a portable, isolated unit that runs identically across environments; the de facto standard container format that Kubernetes/EKS actually schedules and runs (via a CRI-compatible runtime). | Packaging the platform tooling, SageMaker custom training/inference images, and EKS workloads that later get scanned (Aqua) and stored (ECR) across the Data Science platform. | **Strong** — on resume throughout | None. |
| **Amazon ECR** | Elastic Container Registry — a fully managed, private Docker/OCI image registry integrated with IAM for access control, with built-in vulnerability scanning of pushed images; the standard place EKS/ECS pull container images from. | The registry backing the image management lifecycle this JD calls out — where golden/approved container images live before EKS or SageMaker pull them for actual use. | **Moderate** — container work (EKS/Docker) is extensive, but ECR isn't named explicitly anywhere on your resume | Be ready to speak to it directly: it's the private, IAM-integrated container registry your EKS/Docker workflows almost certainly pushed to/pulled from even if the resume doesn't say the name. |
| **AWS Glue** | A serverless data-integration service combining ETL job execution with a Data Catalog — a shared schema registry that S3, Athena, Redshift, and EMR/Spark can all read from; Glue Crawlers scan S3 data and auto-register its schema so tables don't need to be hand-defined. | Cataloging the Data Science platform's S3 data lake so Athena queries and SageMaker training jobs can discover and read data via one shared schema instead of every consumer hand-defining table structure. | **Gap** — not on resume at all, despite Big Data/Spark/EMR being there | Study this specifically — see [§4](#aws-data-lake--analytics-services). It's the natural "next service" alongside your existing EMR/Spark experience. |
| **Amazon Athena** | A serverless SQL query engine that runs directly against data sitting in S3 (via the Glue Data Catalog), with zero infrastructure to provision — billed per query/data scanned, so performance and cost are driven almost entirely by how the underlying data is partitioned and formatted, not by anything you configure at the query-engine layer. | How data scientists/analysts on this platform would query curated datasets ad hoc, directly from S3, without spinning up EMR/Spark for every question. | **Gap** — not on resume | Study this specifically — see [§4](#aws-data-lake--analytics-services) and [§8](#database-technologies-athena-vs-oracle-vs-mysql-vs-postgres). |
| Amazon SageMaker | AWS's full machine-learning lifecycle platform — managed notebooks/Studio IDE, training jobs, hyperparameter tuning, and real-time/serverless/async inference endpoints — for building, training, and hosting custom models rather than just calling a pre-trained foundation model (that's Bedrock's role). | The platform-level service this role provisions and governs access to for a growing Data Science team — Studio Domains, per-user IAM scoping, training job roles, and approved custom training/inference images. | **Strong** — SageMaker on resume (MLOps bullet, AI/ML & HPC Infrastructure section) | Reframe toward *platform support* (provisioning, image management, access) rather than just model training — this JD is platform engineering *for* data scientists, not data science itself. |
| AWS Lambda | Serverless, event-driven compute that runs code in response to a trigger (an API call, an S3 event, a schedule) without provisioning or managing servers, billed per invocation and execution time — commonly used here for environment automation, event-driven data pipeline glue, and lightweight platform-team tooling. | Automating platform operations — e.g., triggering a Glue Crawler when new data lands in S3, enforcing tagging/guardrails on new resources, or responding to a CloudWatch alarm as part of on-call remediation. | **Strong** — on resume | None. |
| **Oracle database** | A traditional enterprise relational database (on-prem, RDS, or Oracle-on-EC2) heavily used in large regulated enterprises for core transactional systems, often predating cloud adoption. Operationally distinct from open-source engines via per-core licensing, Oracle-specific HA/DR tooling (Data Guard, RMAN backups), and dedicated DBA specialization. | Very likely still running core/legacy banking systems at Huntington that the Data Science platform needs to integrate with or extract data from into the data lake. | **Gap** — not on resume at all | Study conceptually — see [§8](#database-technologies-athena-vs-oracle-vs-mysql-vs-postgres). Enterprise banks run Oracle heavily. |
| **MySQL** | An open-source relational database, available fully managed via Amazon RDS — simpler licensing and operational model than Oracle, historically the default choice for web/application backends before Postgres became more common for new workloads. | A plausible RDS engine choice for internal platform tooling or application metadata stores supporting the Data Science platform, distinct from the analytical data lake itself. | **Moderate** — RDS is on resume generically, MySQL isn't named specifically | Know MySQL specifically as an RDS engine option; be ready to discuss it distinctly from Postgres. |
| Amazon RDS (PostgreSQL) | A fully managed relational database service running the PostgreSQL engine, with AWS handling patching and automated backups, and Multi-AZ providing synchronous standby replication and automatic failover — PostgreSQL itself being an open-source relational database increasingly favored for new workloads over MySQL for its richer feature set (JSON support, extensions). | Already your resume's relational database of choice — a plausible backing store for feature stores, experiment-tracking metadata, or general platform metadata supporting the Data Science team. | **Strong** — explicitly on resume | None. |
| **AQUA (Aqua Security)** | A cloud-native security platform focused on containers and cloud workloads — scans container images for vulnerabilities/malware/secrets (often as a CI/CD gate before registry push), enforces Kubernetes security posture, and provides runtime protection against anomalous container behavior in production. | A CI/CD gate scanning every container image (SageMaker custom training/inference images, EKS workload images) before it's allowed into ECR or production, plus ongoing runtime protection once deployed. | **Gap** — not on resume; you have CrowdStrike/Qualys/Tanium instead | Same *category* (container/cloud workload security), different vendor. Study Aqua specifically — see [§7](#security-tooling-for-a-data-science-platform). |
| **Kenna (Kenna Security)** | A risk-based vulnerability prioritization platform (now Cisco Vulnerability Management) — it doesn't scan anything itself; it ingests findings from scanners (Qualys, Tenable) and re-ranks them by real-world exploitability/threat intelligence rather than raw CVSS score, so remediation effort goes to what's actually being exploited in the wild first. | Prioritizing exactly the "resolve infrastructure vulnerabilities within SLAs" responsibility this JD calls out, fed by Qualys/Aqua scan output so the team fixes what's actually being exploited first, not just the highest raw CVSS score. | **Gap** — not on resume | Study this specifically — see [§7](#security-tooling-for-a-data-science-platform). Distinct from the scanners (Qualys) you already know — know that distinction cold. |
| Vulnerability SLAs, audit logging | Vulnerability SLAs: time-bound remediation commitments by severity (e.g., critical in 7 days). Audit logging (CloudTrail/Config): a continuous, immutable record of API activity and configuration state, proving remediation actually happened rather than just that a ticket was closed. | Meeting a bank's regulatory audit requirements directly — CloudTrail/Config evidence paired with documented, SLA-bound remediation timelines is exactly what an examiner or internal audit team asks to see. | **Strong** — CloudTrail, Config, IAM Access Analyzer, Qualys patch management all on resume | Reframe existing vulnerability-remediation work in explicit "SLA" language if asked. |
| **"Data Science Infrastructure" framing** | Not a specific technology — the discipline of building and operating the platform layer (compute, storage, access, governance) that a data science/ML team builds *on top of*, as distinct from doing the data science itself. | How to narrate your existing AI/ML & HPC Infrastructure and Big Data & Analytics resume bullets in interview answers — as evidence of *platform-support* experience, not just infrastructure-in-general experience. | **Moderate** — you have the individual pieces (SageMaker, Bedrock RAG, EMR/Spark, MLOps bullet) but the resume doesn't frame any single role as *platform support for a data science team specifically* | This is a narrative gap, not a skills gap — prepare to explicitly connect your existing experience into "I've supported the infrastructure data scientists actually build on." |
| Image management lifecycle | The end-to-end process governing a base image (AMI or container) from build → security scan → versioned publish → patch/rebuild cadence → formal deprecation, so nothing in production ever launches from an unapproved or stale image. | Governing both AMIs (EC2/SageMaker notebook instances) and container images (ECR) across the Data Science platform's compute, so every workload traces back to a known-good, currently-supported image. | **Moderate** — golden AMIs via EC2 Image Builder (CrowdStrike/Qualys/Tanium integration) already on resume | Reframe this exact resume bullet directly — it *is* an image management lifecycle process, just not labeled that way. See [§6](#image-management-lifecycle). |
| Technical write-ups, 2+ years | Documentation of architecture decisions, runbooks, and operational knowledge in a form other engineers/stakeholders can actually use — the deliverable that turns tribal knowledge into something durable and transferable. | Directly the "conduct root cause analysis and knowledge transfer sessions" responsibility this JD names — RCA docs, runbooks, and onboarding material for a rapidly growing team that can't rely on one person's memory. | **Implicit, not stated on resume** | You have an extensive, real body of technical documentation (this very repo) — be ready to describe it concretely if asked. |
